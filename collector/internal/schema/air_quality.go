...
// BuildRecord creates an Arrow Record for measurements.
// The caller owns the record and must call rec.Release() when done.
func BuildRecord(measurements []Measurement) (arrow.Record, error) {
...
// EncodeToArrow serialises measurements to Arrow IPC stream bytes (for NATS).
func EncodeToArrow(measurements []Measurement) ([]byte, error) {
	rec, err := BuildRecord(measurements)
	if err != nil {
		return nil, err
	}
	defer rec.Release()
	return encodeRecord(AirQualitySchema, rec)
}
